...
class TestTTSService:
    ...
    async def test_initialize_voice_success(
        self, mock_subprocess, mock_piper_voice
    ) -> None:
    ...
    @patch("wave.open")
    async def test_convert_text_to_speech_success(
        self, mock_wave_open, mock_convert_mp3, mock_init_voice
    ) -> None:
        ...
        conversion_id, audio_file = await self.service.convert_text_to_speech(
            text, title
        )
    ...
    @patch.object(TTSService, "initialize_voice")
    @patch("wave.open")
    async def test_convert_text_to_speech_no_title(
        self, mock_wave_open, mock_init_voice
    ) -> None:
